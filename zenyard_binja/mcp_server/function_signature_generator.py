"""
Native-aligned function signature generation.

Faithful port of BinAssistMCP's ``BinaryNinjaFunctionSignatureGenerator`` so the
``get_function_signature`` tool returns the same masked-prefix byte signatures.
"""
...
class BinaryNinjaFunctionSignatureGenerator:
    ...
    def generate(self, function) -> Optional[str]:
        ...
        for address in self._iter_instruction_addresses(function):
            try:
                length = int(
                    self.binary_view.get_instruction_length(address) or 0
                )
            except Exception:
                length = 0
            if length <= 0:
