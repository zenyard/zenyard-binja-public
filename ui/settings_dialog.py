...
class ZenyardSettingsDialog(QDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._auto_apply = QCheckBox("Auto-apply inferences")
        self._auto_apply.setChecked(prefs.auto_apply)
        self._auto_apply.setToolTip(
            "Automatically rename functions and add comments as Zenyard analyzes."
        )
        layout.addWidget(self._auto_apply)

        self._allow_preprocessing = QCheckBox("Allow preprocessing")
        self._allow_preprocessing.setChecked(prefs.allow_preprocessing)
        self._allow_preprocessing.setToolTip(
            "Allow Zenyard to preprocess the binary before analysis."
        )
        layout.addWidget(self._allow_preprocessing)
    ...
    def _test_connection(self) -> None:
        ...
        The HTTP call is made on a background thread (L-07) so that the Binary
        Ninja / Qt UI thread is never blocked.  The button is disabled while the
        test is in progress and UI updates are dispatched back to the main thread
        via execute_on_main_thread_and_wait().
        """
    ...
    def _on_save(self) -> None:
        save_settings(
            ...
            allow_preprocessing=self._allow_preprocessing.isChecked(),
        )
        self.accept()
